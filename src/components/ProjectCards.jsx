...
const ProjectCards = ({image, title, desc, skills, linkGit, linkDemo, linkAndroid, linkIOS}) => {
  return (
    ...
    scrollTrigger-P translate-x-40 odd:-translate-x-40 flex flex-row odd:flex-row-reverse opacity-0'>
        <div className='md:flex hidden w-full h-[fit] justify-center items-start m-4'>
            <a href={linkDemo || linkAndroid || linkIOS || linkGit} target='_blank'>
              <Image3D image={image} height={'350px'} width={'450px'} objectFit={'cover'}/>
            </a>
        </div>
        ...
        <div className='flex flex-col w-full text-secondaryColor justify-center self-center text-center gap-4 h-[350px]'>
            ...
            <div className='text-xl flex justify-around'>
              {linkAndroid && (
                <a className='flex justify-center gap-3 items-center hover:text-thirdColor duration-300 ease-in-out underline underline-offset-2' 
                   href={linkAndroid} target="_blank">
                  Android <BsAndroid2 />
                </a>
              )}
              {linkIOS && (
                <a className='flex justify-center gap-3 items-center hover:text-thirdColor duration-300 ease-in-out underline underline-offset-2' 
                   href={linkIOS} target="_blank">
                  iOS <FaApple />
                </a>
              )}
              {!linkAndroid && !linkIOS && linkDemo && (
                <a className='flex justify-center gap-3 items-center hover:text-thirdColor duration-300 ease-in-out underline underline-offset-2'
                   href={linkDemo} target="_blank">
                  Live Demo <BiLinkExternal />
                </a>
              )}
              {linkGit && (
                <a className='flex justify-center gap-3 items-center hover:text-thirdColor duration-300 ease-in-out underline underline-offset-2' 
                   href={linkGit} target="_blank">
                  GitHub <BsGithub />
                </a>
              )}
            </div>
